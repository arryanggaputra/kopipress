...
<li id="comment-<?php comment_ID();?>" <?php comment_class();?>>
    <div class="comment-body">
        <?php _e('Pingback:', 'kopidev'); // WPCS: XSS OK. ?> <?php comment_author_link();?>
        <?php edit_comment_link(__('Edit', 'kopidev'), '<span class="edit-link">', '</span>');?>
    </div>
...
<li id="comment-<?php comment_ID();?>" <?php comment_class(empty($args['has_children']) ? '' : 'parent');?>>
    <article id="div-comment-<?php comment_ID();?>" class="comment-body" <?php generate_do_microdata('comment-body');?>>
        <footer class="comment-meta flex items-center">
            <?php
        ...
        ?>
            <div class="comment-author-info">
                <div class="comment-author vcard" <?php generate_do_microdata('comment-author');?>>
                    <?php printf('<cite itemprop="name" class="font-bold not-italic">%s</cite>', get_comment_author_link());?>
                </div><!-- .comment-author -->

                <div class="entry-meta comment-metadata mt-0">
                    <a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
                        <time datetime="<?php comment_time('c');?>" itemprop="datePublished">
                            <?php printf( // WPCS: XSS OK.
            /* translators: 1: date, 2: time */
            _x('%1$s at %2$s', '1: date, 2: time', 'kopidev'),
            get_comment_date(),
            get_comment_time()
        );?>
                        </time>
            ...
            <p class="comment-awaiting-moderation">
                <?php _e('Your comment is awaiting moderation.', 'kopidev'); // WPCS: XSS OK. ?></p>
            <?php endif;?>
        </footer><!-- .comment-meta -->

        <div class="comment-content mt-5" itemprop="text">
            <?php
/**
...
function generate_set_comment_form_defaults($defaults)
{
    ...
    $defaults['title_reply']          = apply_filters('generate_leave_comment', __('Leave a Comment', 'kopidev'));
    $defaults['label_submit']         = apply_filters('generate_post_comment', __('Post Comment', 'kopidev'));
